...
##metrics = False (Default; returns lucid algorithm classfication) 
##metrics = True
...
def classify(blob, metrics = False):
    # A quick wrapper method for ease of use
    b = Blob(blob)
    return b.classify(metrics)

def classify_masked(blob, metrics = False):
